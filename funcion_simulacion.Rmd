...
vbin.fit <- fit.variogram(vbin,vgm(c("Gau")), fit.kappa = TRUE)
```
...
return(list("matrix"=mu_sigma,"ubi"=grid))
...
#-----------------------#
mse_ok_idw <- function(mu_sigma,grid){
...
#----------------------#
resultados <- matrix(0,nrow=length(ns),ncol=3)
colnames(resultados) <- c("n","KO","IDW")
resultados[,1] <- ns
j <- 1
for (i in ns) {
  
  n_sim <- OK_IDW_n_values(n=i,contorno=contorno,datos_t=datos_t,datos_tt=datos_tt,vbin.fit=vbin.fit)
  g <- future_replicate(500, expr = mse_ok_idw(mu_sigma=n_sim$matrix,grid=n_sim$ubi))
  resultados[j,c("KO","IDW")] <- apply(g, 1, mean)
  j <- j + 1
}

```
```{r}
save(resultados, file = "resultados2.RData")
```
